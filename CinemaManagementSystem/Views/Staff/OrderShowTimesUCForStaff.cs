...
{
    public partial class OrderShowTimesUCForStaff : UserControl
    {
        private List<Phim> movies = new List<Phim>();
        private string staffId;
        private Panel homepage;

        public OrderShowTimesUCForStaff(string staffId, Phim movie, Panel homepage)
        {
            InitializeComponent();
        ...
        private void lvLichChieu_Click(object sender, EventArgs e)
        {
            if (lvLichChieu.SelectedItems.Count > 0)
            {
                ...
                OrderSeatUCForStaff orderSeatUC = new OrderSeatUCForStaff(showTimes, movie, staffId, homepage);
                orderSeatUC.Dock = DockStyle.Fill;
                homepage.Controls.Clear();
                homepage.Controls.Add(orderSeatUC);
                this.Hide();
                this.OnLoad(null);
                this.Show();
            }
